<aside id="aside">
	<?php if(istOption('recommend_widget')&&istOption('recommend_widget')){$hotWidget=istOption('widgetInfo');}else{$hotWidget=Null;}if(istOption('adcode_sidebar')){$sidebarad=istOption('adcode_sidebar');$sidebarshow=istOption('ad_show_sidebar');}else{$sidebarad=NULL;}if($hotWidget){?><div class="showidget"><?php echo $hotWidget;?></div><?php }
	if(!dynamic_sidebar()){?>
  ...
  <?php if(is_home()&&function_exists('get_most_viewed')){?>
	...
	<?php }if($sidebarad){if($sidebarshow){if(!is_user_logged_in()){echo "<section style=\"padding:10px;\">\n".$sidebarad."</section>\n";}}else{echo "<section style=\"padding:10px;\">\n".$sidebarad."</section>";}}?>
</aside>