...
function add_post_footer_googleplusone( $text ){
	...
	if (
		...
		if (get_option( 'googleplusone_htmlfive' ) == 1){
			$iframe='<div class="g-plusone" data-href="'.trim(get_permalink( $post->ID )).'" '.$size.' '.$displaycount.' '.$jscallback.' '.$width.'></div>';
		} else {
			$iframe='<g:plusone href="'.trim(get_permalink( $post->ID )).'" '.$size.' '.$displaycount.' '.$jscallback.' '.$width.'></g:plusone>';	
		} // if
